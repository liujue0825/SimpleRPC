...
public class ExtensionLoader<T> {
    ...
    /**
     * 扩展类加载器缓存，key - class，value - 对应的扩展器类
     */
    ...
    public static <T> ExtensionLoader<T> getExtensionLoader(Class<T> type) {
        ...
        if (type.getAnnotation(SPI.class) == null) {
            throw new IllegalArgumentException(
                    String.format("Extension type (%s) is not an extension, " +
                            "because it is NOT annotated with @%s!", type, SPI.class.getSimpleName()));
        }
    ...
    private Map<String, Class<?>> getExtensionClasses() {
        ...
        // 如果为空，使用双从检查去加载并缓存
        if (classes == null) {
            // 锁定当前 cachedClasses 的 holder（此时为互斥资源）
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!line.isEmpty()) {
                    try {
                        ...
                        log.error("Failed to load extension class (interface: {}, class line: {}) in {}, cause: {}",
                                type, line, resourceUrl, e.getMessage(), e);
                    }
                }
            }
        } catch (IOException e) {
            log.error("Exception occurred when loading extension class (interface: {}, class file: {}) in {}",
                    type, resourceUrl, resourceUrl, e);
