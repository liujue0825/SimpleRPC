...
public class ClientProxyFactory {
    ...
    public ClientProxyFactory(ServiceDiscovery serviceDiscovery, RpcClient rpcClient, RpcClientProperties rpcClientProperties) {
    ...
    public <T> T getProxy(Class<T> clazz, String version) {
        return (T) CLIENT_PROXY_CACHE.computeIfAbsent(ServiceUtil.getServiceName(clazz.getName(), version), serviceName -> {
            ...
            if (clazz.isInterface() || Proxy.isProxyClass(clazz)) {
                ...
                // 设置方法拦截器
                enhancer.setCallback(new ClientMethodInterceptor(serviceDiscovery, rpcClient, serviceName, rpcClientProperties));
                // 创建代理类
