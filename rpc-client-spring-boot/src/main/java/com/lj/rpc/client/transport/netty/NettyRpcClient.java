...
public class NettyRpcClient implements RpcClient {
     ...
     * 建立连接得到元 channel 对象
     *
     * @param socket 服务端 socket 地址
     * @return channel 对象
     */
    public Channel doConnect(InetSocketAddress socket) throws ExecutionException, InterruptedException {
        CompletableFuture<Channel> channelCompletableFuture = new CompletableFuture<>();
        bootstrap.connect().addListener((ChannelFutureListener) future ->{
            if (future.isSuccess()) {
                log.debug("The client has successfully connected to server [{}]!", socket.toString());
                channelCompletableFuture.complete(future.channel());
            } else {
                throw new RpcException(String.format("The client failed to connect to [%s].", socket.toString()));
            }
        });
        Channel channel = channelCompletableFuture.get();
        channel.closeFuture().addListener(future -> {
            log.info("The client has been disconnected from server [{}].", socket.toString());
        });
        return channel;
    }
